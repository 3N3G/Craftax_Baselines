...
QWEN35_VLLM_OFFLINE_SPLIT_VARIANTS="${QWEN35_VLLM_OFFLINE_SPLIT_VARIANTS:-1}"
...
PY
  ...
  echo "[step] patch vLLM example hidden connector metadata clearing (vLLM ${VLLM_RUNTIME_VERSION})"
  python3 - <<'PY'
import os
import pathlib
import sys
...
for i, line in enumerate(lines):
    if line.startswith("    def clear_connector_metadata(self):"):
        j = i + 1
        while j < len(lines):
            cur = lines[j]
            if cur.startswith("    def ") and not cur.startswith("        "):
                break
            j += 1
        lines[i:j] = [
...
if patched:
    conn_path.write_text("".join(lines), encoding="utf-8")
else:
    # Do not hard-fail if upstream moved/renamed this method.
    print(f"warn_clear_connector_metadata_missing:{conn_path}")

print(f"patched_example_hidden_connector_clear_metadata={conn_path}")
PY
...
run_qwen_vllm_offline_bench() {
  ...
  if [[ "${QWEN35_VLLM_OFFLINE_SPLIT_VARIANTS}" == "1" ]]; then
    local split_root="${bench_out}/_split_variant_runs"
    local state_out="${split_root}/predict_state_only"
    local history_out="${split_root}/predict_history_k5"
    local empty_prompt_dir="${split_root}/empty_prompts"

    mkdir -p "${state_out}" "${history_out}" "${empty_prompt_dir}"
    ...
    run_python scripts/benchmark_future_imagination_latency.py \
      ...
      "${VLLM_OFFLINE_ARGS[@]}" \
      --prompt-dir-state "${PROMPT_STATE_DIR}" \
      --prompt-dir-history "${empty_prompt_dir}" \
      --benchmark-mode "${BENCHMARK_MODE}" \
      --warmup-prompts-per-variant "${WARMUP_PROMPTS_PER_VARIANT}" \
      --max-tokens "${MAX_TOKENS}" \
      --temperature "${TEMPERATURE}" \
    ...
    run_python scripts/benchmark_future_imagination_latency.py \
      ...
      --vllm-kv-connector-mode "${QWEN35_VLLM_OFFLINE_CONNECTOR_MODE}" \
      "${VLLM_OFFLINE_ARGS[@]}" \
      --prompt-dir-state "${empty_prompt_dir}" \
      --prompt-dir-history "${PROMPT_HISTORY_DIR}" \
      --benchmark-mode "${BENCHMARK_MODE}" \
      --warmup-prompts-per-variant "${WARMUP_PROMPTS_PER_VARIANT}" \
      --max-tokens "${MAX_TOKENS}" \
      ...
      --timeout-s "${REQUEST_TIMEOUT_S}" \
      --hidden-target-layer -1 \
      --output-dir "${history_out}" \
      --run-label "${label}__history_k5"

    run_python - <<'PY' "${bench_out}" "${state_out}" "${history_out}" "${PROMPT_STATE_DIR}" "${PROMPT_HISTORY_DIR}" "${label}"
import json
import sys
from pathlib import Path

bench_out = Path(sys.argv[1])
state_out = Path(sys.argv[2])
history_out = Path(sys.argv[3])
prompt_state_dir = sys.argv[4]
prompt_history_dir = sys.argv[5]
label = sys.argv[6]

sys.path.insert(0, str(Path.cwd()))
from scripts.benchmark_future_imagination_latency import _summarize, _write_jsonl, _write_markdown

records = []
for child in (state_out, history_out):
    records_path = child / "records.jsonl"
...
meta = {}
meta_path = state_out / "run_meta.json"
if meta_path.exists():
...
meta.update(
    {
        ...
        "prompt_dir_state": prompt_state_dir,
        "prompt_dir_history": prompt_history_dir,
        "split_variant_runs": [str(state_out), str(history_out)],
        "split_variants_enabled": True,
    }
)

bench_out.mkdir(parents=True, exist_ok=True)
_write_jsonl(bench_out / "records.jsonl", records)
(bench_out / "summary.json").write_text(
    json.dumps(summary, indent=2), encoding="utf-8"
)
(bench_out / "run_meta.json").write_text(
    json.dumps(meta, indent=2), encoding="utf-8"
)
_write_markdown(bench_out / "summary.md", run_meta=meta, summary=summary)

print(f"merged_records_jsonl={bench_out / 'records.jsonl'}")
print(f"merged_summary_json={bench_out / 'summary.json'}")
print(f"merged_summary_md={bench_out / 'summary.md'}")
print(f"merged_calls={len(records)}")
print(f"merged_ok={sum(1 for r in records if r.get('status') == 'ok')}")
print(f"merged_errors={sum(1 for r in records if r.get('status') != 'ok')}")
PY
  else
    run_python scripts/benchmark_future_imagination_latency.py \
      --provider vllm_offline \
