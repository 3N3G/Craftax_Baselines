...
def get_hidden_state():
    ...
    try:
        ...
        if current_request_id <= 3 or DEBUG_MODE:
            num_subsampled = last_layer_hidden_state.shape[1]
            print(
                f"[DIAG] After subsampling: {last_layer_hidden_state.shape} "
                f"(training data has 80 tokens, got {num_subsampled})"
            )
            raw_l2_before_pool = torch.norm(last_layer_hidden_state, p=2).item()
