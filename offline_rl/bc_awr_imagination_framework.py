...
class ActorCriticAug(nn.Module):
    """Same augmented architecture used by offline AWR and online JAX runs."""
    ...
    def __init__(
        ...
        self.actor_obs_fc1 = nn.Linear(self.obs_dim, self.layer_width)
        self.actor_hidden_fc1 = nn.Linear(self.hidden_state_dim, self.layer_width)
        self.actor_fc1 = nn.Linear(2 * self.layer_width, self.layer_width)
        self.actor_fc2 = nn.Linear(self.layer_width, self.layer_width)
        self.actor_out = nn.Linear(self.layer_width, self.action_dim)

        self.critic_obs_fc1 = nn.Linear(self.obs_dim, self.layer_width)
        self.critic_hidden_fc1 = nn.Linear(self.hidden_state_dim, self.layer_width)
        self.critic_fc1 = nn.Linear(2 * self.layer_width, self.layer_width)
        self.critic_fc2 = nn.Linear(self.layer_width, self.layer_width)
        self.critic_out = nn.Linear(self.layer_width, 1)

        self.apply_init()

    def apply_init(self) -> None:
        orthogonal_init(self.actor_obs_fc1, gain=math.sqrt(2.0))
        orthogonal_init(self.actor_hidden_fc1, gain=math.sqrt(2.0))
        orthogonal_init(self.actor_fc1, gain=math.sqrt(2.0))
        orthogonal_init(self.actor_fc2, gain=math.sqrt(2.0))
        orthogonal_init(self.actor_out, gain=0.01)

        orthogonal_init(self.critic_obs_fc1, gain=math.sqrt(2.0))
        orthogonal_init(self.critic_hidden_fc1, gain=math.sqrt(2.0))
        orthogonal_init(self.critic_fc1, gain=math.sqrt(2.0))
        orthogonal_init(self.critic_fc2, gain=math.sqrt(2.0))
    ...
    def forward(
        ...
        if hidden_state is None:
            raise ValueError("ActorCriticAug requires hidden_state inputs.")
