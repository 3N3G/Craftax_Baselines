...
- **`labelling/`**: Distributed trajectory labelling infrastructure (vLLM-based)
  - `llm_worker.py`: Main worker (extracts last-token hidden states via vLLM)
  - `addtoqueue_llm.py`: Queue files for processing
...
- `hidden_state`: (augmented only) Last-token hidden state (2560,) from Qwen3-4B via vLLM
...
1. **Generation**: PPO training with `--save_trajectory` flag saves rollout data to `/data/group_data/rl/geney/craftax_unlabelled_symbolic/`
2. **Labelling**: Run `sbatch labelling/run_labelling.sbatch` which:
   - Starts Redis coordinator, queues pending files
   - Spawns GPU workers (`makeworkers_llm.sbatch`), each starts its own vLLM server
   - Workers run `llm_worker.py` to extract last-token hidden states via vLLM
   - Labelled data saved to `/data/group_data/rl/geney/craftax_llm_labelled_results/`
3. **Re-extraction**: To add hidden states to existing files, use `sbatch labelling/run_extract_hidden.sbatch`
4. **Analysis**: `collectdatasetstats.py` computes dataset statistics

The Redis queue ensures fault tolerance - if workers die, `janitor_llm.py` re-enqueues orphaned jobs.
...
- Queue name: `craftax_llm_job_queue`
- Redis hostname written to `/data/group_data/rl/geney/redis_host.txt` by coordinator
- `janitor_llm.py` monitors for orphaned jobs and re-queues them
...
**Note**: The current pipeline uses `llm_worker.py` with vLLM for fast hidden state extraction.
Hidden states are saved as last-token only: shape `(N, 2560)`.
