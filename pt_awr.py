...
class OfflineDataset:
    ...
    def sample(self, batch_size):
        ...
        next_obs_t = torch.tensor(self.next_obs[idx], dtype=torch.float32, device=Config.DEVICE) / 255.0
        action_t = torch.tensor(self.action[idx], dtype=torch.long, device=Config.DEVICE)
        reward_t = torch.tensor(self.reward[idx], dtype=torch.float32, device=Config.DEVICE)
        ...
        return {
            ...
            "reward": reward_t, "done": done_t, "return_to_go": rtg_t
        }
...
def draw_dual_line_graph(frame, val_hist, ret_hist, y_min, y_max, total_steps):
    ...
    # Text Legend
    cv2.putText(new_frame, f"Pred: {curr_v:.1f}", (10, h + 20), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0), 1)
    cv2.putText(new_frame, f"True: {curr_r:.1f}", (10, h + 40), 
...
def run_eval(model, step):
    ...
    for ep in range(Config.EVAL_EPISODES):
        ...
        while not done and step_count < 2000:
            ...
            if obs_tensor.max() > 1.1: obs_tensor /= 255.0
        ...
        if capture_video:
            ...
            print(f"Generating video. Steps: {total_steps}, Y-Range: [{y_min:.1f}, {y_max:.1f}]")
            
            for i in range(len(ep_raw_frames)):
                # We pass 'total_steps' so the graph knows the final width
                vf = draw_dual_line_graph(ep_raw_frames[i], 
                                          ep_values[:i+1], 
                                          true_rtgs[:i+1], 
                                          y_min, y_max,
                                          total_steps) # <--- NEW ARGUMENT
...
def parse_args():
    parser = argparse.ArgumentParser(description="Advantage Weighted Regression for Craftax")
    parser.add_argument("--data_dir", type=str, default=Config.DATA_DIR, help="Path to training data")
    ...
    parser.add_argument("--awr_beta", type=float, default=Config.AWR_BETA, help="AWR temperature parameter")
    parser.add_argument("--seed", type=int, default=Config.SEED, help="Random seed")
    parser.add_argument("--wandb_name", type=str, default=Config.WANDB_NAME, help="WandB run name")
    parser.add_argument("--eval_freq", type=int, default=Config.EVAL_FREQ, help="Evaluation frequency")
    parser.add_argument("--save_freq", type=int, default=Config.SAVE_FREQ, help="Checkpoint save frequency")
...
def main():
    ...
    model = ActorCriticConv(action_dim=Config.ACTION_DIM, layer_width=Config.LAYER_WIDTH).to(Config.DEVICE)
    ...
    for step in range(1, Config.TOTAL_STEPS + 1):
        ...
        if step % Config.LOG_FREQ == 0:
            ...
            if step % (Config.LOG_FREQ * 10) == 0:
                print(f"Step {step}/{Config.TOTAL_STEPS}: actor_loss={metrics['actor_loss']:.4f}, "
